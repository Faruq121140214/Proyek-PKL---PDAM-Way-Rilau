        ...
        <div class="flex flex-1 pt-32" x-data="{ modalLogout: false}" x-cloak>
            ...
            <aside id="sidebar" class="sidebar w-64 bg-white h-[calc(100vh-5.9rem)] fixed top-19 left-0 z-10 pt-4">
                <nav class="text-black text-base font-semibold px-4 py-2">
                    <!-- Sidebar links -->
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center py-2.5 @if (str_starts_with(Route::currentRouteName(), 'admin.dashboard'))
                    ...
                    <a href="{{ route('admin.absensi.kelola-absensi') }}" class="flex items-center py-2.5 @if (str_starts_with(Route::currentRouteName(), 'admin.absensi'))
                     border-l-4 pl-3.5 rounded-l border-current  text-black font-bold @else px-4 rounded hover:bg-black hover:text-white @endif transition duration-300">
                        <i class="fa-solid fa-clock mr-2"></i>
                      <span class="overflow-hidden text-ellipsis whitespace-nowrap">Data Absensi</span>
                    </a>
                    <a href="{{ route('admin.mahasiswa.kelola-mahasiswa') }}" class="flex items-center py-2.5 @if (str_starts_with(Route::currentRouteName(), 'admin.mahasiswa'))
                     border-l-4 pl-3.5 rounded-l border-current  text-black font-bold @else px-4 rounded hover:bg-black hover:text-white @endif transition duration-300">
                        <i class="fa-solid fa-users mr-2"></i>
                      <span class="overflow-hidden text-ellipsis whitespace-nowrap">Data Mahasiswa</span>
                    </a>
                    <a href="{{ route('admin.profile') }}" class="flex items-center py-2.5 @if (str_starts_with(Route::currentRouteName(), 'admin.profile'))
                     border-l-4 pl-3.5 rounded-l border-current  text-black font-bold @else px-4 rounded hover:bg-black hover:text-white @endif transition duration-300">
                      <i class="fa-solid fa-user mr-2"></i>
                      <span class="overflow-hidden text-ellipsis whitespace-nowrap">Profile</span>
                    </a>
                    <a href="#" @click="modalLogout = true" class="flex items-center py-2.5 @if (str_starts_with(Route::currentRouteName(), 'superadmin.rekening'))
                     border-l-4 pl-3.5 rounded-l border-current  text-black font-bold @else px-4 rounded hover:bg-black hover:text-white @endif transition duration-300">
                ...
                <div @click.away="modalLogout = false" class="modal-container bg-white w-11/12 md:max-w-2xl mx-auto rounded-2xl shadow-lg overflow-y-auto z-50">
                    <div class="modal-content py-4 text-left px-8">
                        <div class="flex justify-between items-center pb-6">
                            ...
                            <div class="modal-close cursor-pointer z-50" @click="modalLogout = false">
                                <svg class="fill-current text-black" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18">
                                    <path d="M14.53 3.47a.75.75 0 0 0-1.06-1.06L9 6.94 4.53 2.47a.75.75 0 0 0-1.06 1.06L7.94 9l-4.47 4.47a.75.75 0 0 0
                                    1.06 1.06L9 11.06l4.47 4.47a.75.75 0 0 0 1.06-1.06L10.06 9l4.47-4.47z" />
                                </svg>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-3 items-center justify-end">
                            ...
                            <button @click="modalLogout=false" id="closeModalButton" class="bg-gray-500 text-white border border-gray-300 px-4
                            py-2 rounded-lg mb-6 mt-2" type="button">Batal</button>
                        </div>
                    </div>
                </div>
            </div>
